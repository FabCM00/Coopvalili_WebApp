...
import { type SolicitudResumen } from "@/lib/types";
import { type SolicitudEstado, ESTADO_LABEL, ESTADO_DOT } from "@/lib/bandeja";
import { ChevronLeft, ChevronRight } from "lucide-react";

interface RequestTableProps {
  rows: SolicitudResumen[];
            ...
            {loading ? (
              <>
                {Array.from({ length: 8 }).map((_, i) => (
                  <tr
                    key={i}
                    className="border-b border-[#0D0D0D]/5 animate-pulse"
                  >
                    {[80, 72, 120, 140, 72, 80, 48].map((w, j) => (
                      <td key={j} className="px-4 py-3">
                        <div
                          className="h-3.5 bg-[#0D0D0D]/8 rounded"
                          style={{ width: w }}
                        />
                      </td>
                    ))}
              ...
              <tr>
                <td
                  colSpan={7}
                  className="py-12 text-center text-sm text-[#0D0D0D]/35"
                >
              ...
              rows.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-xs text-[#0D0D0D]/55">
                      {formatFecha(s.fecha)}
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-[#0D0D0D]/55">
                      {s.radicado}
                    </td>
                    <td className="px-4 py-3 text-[#0D0D0D]/85">
                      {s.solicitante}
                    </td>
...
function ScoreChip({
  ...
  const tone =
    estado === "no_valida_1" ||
    estado === "no_val_identidad" ||
    estado === "no_viable" ||
    estado === "fallo_servicios"
